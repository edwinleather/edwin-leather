...
export function parseDataUri(dataUri: string): { mime: string; base64: string } {
  ...
  if (bytes > MAX_BYTES) throw new ApiError(413, "Image too large (max 10MB).");
  return { mime, base64 };
}

export async function uploadImage(dataUri: string, folder = "edwin/products"): Promise<{ url: string; publicId: string }> {
  ensureConfigured();
