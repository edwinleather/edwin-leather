      ...
      {items.map((item, idx) => (
        ...
        >
          <AnimatePresence>
            {hoveredIndex === idx && (
              <motion.span
                className="absolute inset-0 h-full w-full bg-paper-2 block rounded-3xl"
...
export const Card = ({
  ...
  return (
    <div
      className={cn(
        "rounded-2xl h-full w-full p-4 overflow-hidden bg-paper border border-line group-hover:border-tan relative z-20",
        className
      )}
    >
...
export const CardTitle = ({
  ...
  return (
    <h3 className={cn("text-sm font-medium tracking-wide mt-2", className)}>
      {children}
    </h3>
  );
};
