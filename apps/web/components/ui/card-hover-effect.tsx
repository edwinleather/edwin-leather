      ...
      {items.map((item, idx) => (
        ...
        >
          <AnimatePresence>
            {hoveredIndex === idx && (
              <motion.span
                className="absolute inset-0 h-full w-full bg-paper-2 dark:bg-slate-800/[0.8] block rounded-3xl"
...
export const Card = ({
  ...
  return (
    <div
      className={cn(
        "rounded-2xl h-full w-full p-4 overflow-hidden bg-paper border border-line dark:border-white/[0.2] group-hover:border-tan relative z-20",
        className
      )}
    >
