...
import type { BackofficeRequest } from "../middleware/backoffice";
...
import { AdminUser, RolePermission, ADMIN_ROLES } from "../models/backoffice";
import { adminPublic, allFeatures, DEFAULT_FEATURES } from "../services/backoffice";
import { getDeliveryConfig, invalidateDeliveryConfigCache } from "../services/delivery";
import { getTaxConfig, invalidateTaxConfigCache } from "../services/tax";
import { getCodConfig, invalidateCodConfigCache } from "../services/cod";
import { getPaymentKeys, invalidatePaymentConfigCache } from "../services/payment-config";
...
import { PAGE_KEYS, defaultPageContent } from "../services/pages";
import { exportAllDatabases, importAllDatabases } from "../services/databaseExport";

export const adminRouter = Router();
// Shared router constant: also exported as backofficeRouter for legacy imports.
export const backofficeRouter = adminRouter;

// Shared superadmin guard (moved up from the legacy block: routes in both
// blocks must reference it after it is declared).
const requireSuperadmin = requireBackofficeRole("superadmin");

// ─── Legacy backoffice block (moved verbatim from server/routes/backoffice.ts).
// Registered on the shared router, so the single /api/v1/admin mount serves it.
...
adminRouter.get("/settings", requireBackofficeAdmin, requireBackofficeFeature("homepage"), async (_req, res, next) => {
  try {
...
adminRouter.put("/settings", requireBackofficeAdmin, requireBackofficeFeature("homepage"), async (req, res, next) => {
  try {
...
adminRouter.get("/stats", requireBackofficeAdmin, async (_req, res, next) => {
  try {
...
adminRouter.get("/delivery", requireBackofficeAdmin, requireBackofficeFeature("shipping"), async (_req, res, next) => {
  try {
...
adminRouter.put("/delivery", requireBackofficeAdmin, requireBackofficeFeature("shipping"), async (req, res, next) => {
  try {
    const input = z
      .object({
...
adminRouter.get("/tax", requireBackofficeAdmin, requireBackofficeFeature("taxes"), async (_req, res, next) => {
  try {
...
adminRouter.put("/tax", requireBackofficeAdmin, requireBackofficeFeature("taxes"), async (req, res, next) => {
  try {
    const input = z
      .object({
...
adminRouter.get("/cod", requireBackofficeAdmin, requireBackofficeFeature("shipping"), async (_req, res, next) => {
  try {
...
adminRouter.put("/cod", requireBackofficeAdmin, requireBackofficeFeature("shipping"), async (req, res, next) => {
  try {
...
adminRouter.get("/payment-mode", requireBackofficeAdmin, async (_req, res, next) => {
  try {
...
adminRouter.put("/payment-mode", requireBackofficeAdmin, requireSuperadmin, async (req, res, next) => {
  try {
...
adminRouter.get("/me", requireBackofficeAdmin, async (req, res, next) => {
  try {
...
adminRouter.get("/users", requireSuperadmin, async (_req, res, next) => {
  try {
...
adminRouter.patch("/users/:userId", requireSuperadmin, async (req, res, next) => {
  try {
...
adminRouter.delete("/users/:userId", requireSuperadmin, async (req, res, next) => {
  try {
    const me = (req as BackofficeRequest).admin!.id;
...
adminRouter.get("/roles", requireSuperadmin, async (_req, res, next) => {
  try {
...
adminRouter.put("/roles/:role", requireSuperadmin, async (req, res, next) => {
  try {
...
adminRouter.get("/pages", requireBackofficeAdmin, requireBackofficeFeature("pages"), async (_req, res, next) => {
  try {
...
adminRouter.get("/pages/:key", requireBackofficeAdmin, requireBackofficeFeature("pages"), async (req, res, next) => {
  try {
...
adminRouter.put("/pages/:key", requireBackofficeAdmin, requireBackofficeFeature("pages"), async (req, res, next) => {
  try {
    const key = req.params.key as (typeof PAGE_KEYS)[number];
    if (!PAGE_KEYS.includes(key)) return next(new ApiError(404, "Page not found"));
...
adminRouter.get("/export/database", requireSuperadmin, async (_req, res, next) => {
  try {
...
adminRouter.post("/import/database", requireSuperadmin, async (req, res, next) => {
  try {
    const body = req.body;
